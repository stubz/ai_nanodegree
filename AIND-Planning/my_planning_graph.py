...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if newnode.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(newnode)
                for snode in newnode.prenodes:
                    if(newnode.__eq__(snode)):
                        newnode.add(newnode)
        """
    ...
    def add_literal_level(self, level):
        ...
        for pgna in self.a_levels[level-1]:
            ...
            #self.s_levels[level].add(pgns_set)

            #pgna.children.add(pgns_set)
    ...
    def h_levelsum(self) -> int:
        ...
        def first_level(self):
            for i, s_set in enumerate(self.s_levels):
                for snode in s_set:
                    literal = expr(snode.symbol)
                    if not snode.is_pos:
                        literal = expr('~{}'.format(snode.symbol))
                    if g == literal:
                        return i


        for g in self.problem.goal:
            level_sum += first_level(self)
